...
def train():
    ...
    for batch, i in enumerate(range(0, train_data.size(0) - 1, args.bptt)):
        ...
        if args.model == 'Transformer':
            ...
            hidden = repackage_hidden(hidden)
            output, hidden = model(data, hidden)
        loss_super = criterion(output, targets)
        params = []
        # print(output)
        ...
        if len(grad_list) > 0:
            for iii in range(maxIter):
                ...
                # Hv = torch.autograd.grad(group_product(grad_list, v), params, retain_graph=True, create_graph=True)
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            with open('WT2_Hessian.txt','a',encoding='utf-8') as f:
                f.write('| epoch %d | %d / %d  batches | lr %.2f | ms/batch %.2f | loss %.2f | ppl %.2f \n' % (epoch, batch, len(train_data) // args.bptt, lr, elapsed * 1000 / args.log_interval, cur_loss, math.exp(cur_loss)))
            total_loss = 0
            start_time = time.time()
        if args.dry_run:
            break
...
try:
    for epoch in range(1, args.epochs+1):
        ...
        print('-' * 89)
        with open('WT2_Hessian.txt','a',encoding='utf-8') as f:
            f.write('| end of epoch %d | time: %2f s | valid loss %2f | valid ppl %.2f \n' % (epoch, (time.time() - epoch_start_time), val_loss, math.exp(val_loss)))
...
with open('WT2_Hessian.txt','a',encoding='utf-8') as f:
    f.write('| End of training | valid loss %.2f | valid ppl %.2f \n' % (best_val_loss, math.exp(best_val_loss)))
with open('WT2_Hessian.txt','a',encoding='utf-8') as f:
    f.write('| End of training | test loss {%.2f} | test ppl {%.2f} \n' % (best_test_loss, math.exp(best_test_loss)))
print('=' * 89)
